...
async def privacy_page():
    html_path = DASHBOARD_DIR / "privacy.html"
    if html_path.exists():
        return HTMLResponse(content=_read_dashboard_html("privacy.html"))
    return HTMLResponse(content="<h1>Privacy</h1>")


@app.get("/terms", response_class=HTMLResponse, include_in_schema=False)
async def terms_page():
    html_path = DASHBOARD_DIR / "terms.html"
    if html_path.exists():
        return HTMLResponse(content=_read_dashboard_html("terms.html"))
    return HTMLResponse(content="<h1>Terms</h1>")


@app.get("/status", response_class=HTMLResponse, include_in_schema=False)
async def status_page():
    html_path = DASHBOARD_DIR / "status.html"
    if html_path.exists():
        return HTMLResponse(content=_read_dashboard_html("status.html"))
